...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val hasData = sobrieTree?.getAllEntries()?.isNotEmpty() ?: false
    ...
    private fun initializeSobrieTree() {
        try {
            sobrieTree = SobrieTreeProvider.instance
    ...
    private fun restoreGoalsAndBaseline() {
        ...
        sobrieTree?.setEndOfDayHour(eod)
        ...
        if (goalDaily > 0.0 || goalWeekly > 0.0) {
            ...
            sobrieTree?.setConsumptionGoal(goalDaily, goalWeekly, today, today.plusWeeks(4))
        }
        val baselineDaily = prefs.getFloat("baseline_daily_ml", 0f).toDouble()
        if (baselineDaily > 0.0) {
            val today = LocalDate.now()
            sobrieTree?.setBaseline(startDate = today, endDate = today.plusWeeks(4), totalConsumption = null, dailyAverage = baselineDaily)
        } else {
            ...
            val today = LocalDate.now()
            sobrieTree?.setBaseline(startDate = today, endDate = today.plusWeeks(4), totalConsumption = null, dailyAverage = defaultDailyBaselineMl)
    ...
    private fun showMilestoneCelebrations(badges: List<com.sobrietree.android.engine.Badge>) {
