 ...
 *
 * SobrieTree keeps everything on the device and takes no network permission, so
 ...
 * and reveals nothing beyond "this is a SobrieTree backup".
 */
object BackupCrypto {
     ...
     *
     * Deliberately not renamed with the app. This is the only route data has
     * between installs, and the app has already changed identity twice; a
     * signature that follows the branding would strand every backup written
     * under the old name at exactly the moment it was needed. It identifies the
    ...
    fun decrypt(blob: ByteArray, passphrase: CharArray): ByteArray {
        if (blob.size <= headerSize) throw BackupFormatException("Not a SobrieTree backup file.")
        if (!blob.copyOfRange(0, MAGIC.size).contentEquals(MAGIC)) {
            throw BackupFormatException("Not a SobrieTree backup file.")
        }
        val version = blob[MAGIC.size]
        if (version != VERSION) {
            throw BackupFormatException("This backup was written by a newer version of SobrieTree.")
        }
     ...
     * "PBKDF2WithHmacSHA256" only exists from API 26, and SobrieTree supports 24.
