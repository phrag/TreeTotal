 ...
 * TreeTotal; [onSaved] fires after a successful save (callers handle any
 * onboarding chaining themselves).
 */
object GoalsSetupDialog {

    fun show(activity: Activity, onSaved: () -> Unit) {
        val treeTotal = TreeTotalProvider.instance
        ...
        val currentDailyMl = treeTotal.getDailyGoal()
        val currentWeeklyMl = treeTotal.getWeeklyGoal()
        ...
        val baseline = treeTotal.getCurrentBaseline()
        ...
        dialogView.findViewById<View>(R.id.btn_save).setOnClickListener {
            ...
            treeTotal.setConsumptionGoal(mlDailyGoal, mlWeeklyGoal, today, today.plusWeeks(4))
            treeTotal.setBaseline(startDate = today, endDate = today.plusWeeks(4), totalConsumption = null, dailyAverage = mlDailyBaseline)
